...
class Micelle(torch.nn.Module):
  ...
  def __init__(self, config):
    ...
    self.inner_shell_ratio = config.inner_shell_ratio
    self.shell_density_ratio = config.shell_density_ratio
  ...
  def get_micelle(self, rot_params, micelle_params):
    ...
    for idx in range(batch_size): # TODO: vectorize
      micelles[idx, 0, :, :] = two_phase_micelle(x_mesh, y_mesh,
                                ...
                                inner_shell_ratio=self.inner_shell_ratio,
                                shell_density_ratio=self.shell_density_ratio)
    return micelles
